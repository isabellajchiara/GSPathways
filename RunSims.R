...
script = "SimplifiedBreedingCyclePipeline.R"
Reps = as.vector(rep(script,args$nReps))
...
if (args$nCores == 1 || modelParallelism) { # Run reps serially
  ...
  res <- parLapply(cl, Reps, function(X){
    source(X) ##Source the SCript for the SCenario you would like to run##
    ret
  })
